		...
		>
			{#each Array(5) as boardSlot, index}
				<div
					...
					class="boardItem border {actionState.moveCardState
						? 'hover:bg-white/20'
						: ''}
					{isEnemy
						? 'border-red-500'
...
<div
	class="board h-dvh w-dvw flex relative justify-center gap-8 flex-col items-center"
	onmouseup={() => {
	...
	onclick={(e) => {
		if (
			actionState.selectedCard &&
			actionState.isCardClicked &&
...
>
	...
	<h1 class="text-4xl absolute pointer-events-none text-primary/50">
		Spirit Fighter
	</h1>
	{@render playerBoard()}
</div>
